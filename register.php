...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // รับค่าจากฟอร์ม
  // **หมายเหตุ: เรายังรับค่า username มาเพื่อ validation แต่จะไม่ได้ใช้บันทึกลง DB**
  $username  = trim($_POST['username'] ?? "");
  $password  = $_POST['password'] ?? "";
  $email     = trim($_POST['email'] ?? "");
  $full_name = trim($_POST['name'] ?? ""); // ค่านี้จะถูกใช้สำหรับคอลัมน์ display_name

  // ตรวจความถูกต้องเบื้องต้น (คงไว้เหมือนเดิม)
...
// ตรวจซ้ำ email และ display_name
  if (!$errors) {
    // FIX 1: แก้ SQL ให้ตรวจทั้ง email และ display_name
    $sql = "SELECT 1 FROM users WHERE email = ? OR display_name = ? LIMIT 1";
    if ($stmt = $mysqli->prepare($sql)) {
      // FIX 2: แก้ bind_param ให้ส่ง 2 ค่า (email และ full_name)
      $stmt->bind_param("ss", $email, $full_name);
      $stmt->execute();
      $stmt->store_result();
      if ($stmt->num_rows > 0) {
        // FIX 3: แก้ไขข้อความ Error ให้ครอบคลุมทั้งสองกรณี
        $errors[] = "Email หรือชื่อที่แสดงนี้ถูกใช้แล้ว";
      }
      $stmt->close();
    } else {
  ...
  if (!$errors) {
    ...
    // FIX 3: ตรวจสอบว่า SQL INSERT ตรงกับตาราง (ซึ่งตรงอยู่แล้ว)
    $sql = "INSERT INTO users (email, password_hash, display_name) VALUES (?, ?, ?)";
    if ($stmt = $mysqli->prepare($sql)) {
      // FIX 4: แก้ bind_param ให้ส่ง 3 ค่าที่ถูกต้อง: email, password_hash, full_name
      ...
      if ($stmt->execute()) {
        ...
        if ($mysqli->errno == 1062) {
          // 1062 คือ Error duplicate entry
          $errors[] = "Email นี้ซ้ำในระบบ กรุณาใช้ค่าอื่น";
        } else {
